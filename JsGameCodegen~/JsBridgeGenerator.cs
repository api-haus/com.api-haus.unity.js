...
namespace JsGameCodegen
{
    ...
    public class JsBridgeGenerator : IIncrementalGenerator
    {
        ...
        {
            return name == JsBridgeAttrShort || name == "JsBridgeAttribute"
                   || name.EndsWith(".JsBridge") || name.EndsWith(".JsBridgeAttribute");
        }
        ...
        static BridgeTarget? ExtractFromStruct(GeneratorSyntaxContext ctx, CancellationToken ct)
        {
            ...
            foreach (var attrData in symbol.GetAttributes())
            {
                ...
                return new BridgeTarget(symbol.Name, symbol.ToDisplayString(), jsName, needAccessors, needSetters, fields, desc);
            }
        ...
        static BridgeTarget? ExtractFromAssemblyAttribute(GeneratorSyntaxContext ctx, CancellationToken ct)
        {
            ...
            return new BridgeTarget(targetType.Name, targetType.ToDisplayString(), jsName, needAccessors, needSetters, fields, desc);
        }
        ...
        static JsFieldType MapFieldType(ITypeSymbol type)
        {
            ...
            switch (name)
            {
                case "float": return JsFieldType.Float;
                case "int": return JsFieldType.Int;
                case "bool": return JsFieldType.Bool;
                case "Unity.Mathematics.float2": return JsFieldType.Float2;
                case "Unity.Mathematics.float3": return JsFieldType.Float3;
                case "Unity.Mathematics.float4": return JsFieldType.Float4;
                case "Unity.Mathematics.quaternion": return JsFieldType.Quaternion;
                default:
        ...
        static void GenerateBridge(SourceProductionContext ctx, BridgeTarget target)
        {
            ...
            var needsXY = fields.Any(f => f.Type == JsFieldType.Float2 || f.Type == JsFieldType.Float3 || f.Type == JsFieldType.Float4 || f.Type == JsFieldType.Quaternion);
            var needsZ = fields.Any(f => f.Type == JsFieldType.Float3 || f.Type == JsFieldType.Float4 || f.Type == JsFieldType.Quaternion);
            var needsW = fields.Any(f => f.Type == JsFieldType.Float4 || f.Type == JsFieldType.Quaternion);
            if (needsXY)
            {
            ...
            if (needAccessors)
            {
                ...
                sb.AppendLine("\t\t\t{");
                sb.AppendLine("\t\t\t\tvar fn = QJSShim.qjs_shim_new_function(ctx, Get_Component, pGet, 1);");
            ...
            if (needSetters)
            {
                ...
                sb.AppendLine("\t\t\t{");
                sb.AppendLine("\t\t\t\tvar fn = QJSShim.qjs_shim_new_function(ctx, Set_Component, pSet, 2);");
            ...
            if (hasAnyDoc)
            {
                ...
                foreach (var field in fields)
                {
                    if (field.Description != null)
                        sb.AppendLine("\t\t\t{ \"" + EscapeString(field.JsName) + "\", \"" + EscapeString(field.Description) + "\" },");
                }
                sb.AppendLine("\t\t};");
            }
        ...
        static void EmitGetComponent(StringBuilder sb, ImmutableArray<FieldInfo> fields, string fullTypeName)
        {
            sb.AppendLine("\t\t[MonoPInvokeCallback(typeof(QJSShimCallback))]");
            sb.AppendLine("\t\tstatic unsafe void Get_Component(JSContext ctx, long thisU, long thisTag,");
            ...
            foreach (var field in fields)
            {
                ...
                switch (field.Type)
                {
                    case JsFieldType.Float:
                        sb.AppendLine("\t\t\t\tQJS.JS_SetPropertyStr(ctx, obj, p_" + field.JsName + ", QJS.NewFloat64(ctx, comp." + field.Name + "));");
                        break;
                    case JsFieldType.Int:
                        sb.AppendLine("\t\t\t\tQJS.JS_SetPropertyStr(ctx, obj, p_" + field.JsName + ", QJS.NewInt32(ctx, comp." + field.Name + "));");
                        break;
                    case JsFieldType.Bool:
                        sb.AppendLine("\t\t\t\tQJS.JS_SetPropertyStr(ctx, obj, p_" + field.JsName + ", QJS.JS_NewBool(ctx, comp." + field.Name + " ? 1 : 0));");
                        break;
                    case JsFieldType.Enum:
                        sb.AppendLine("\t\t\t\tQJS.JS_SetPropertyStr(ctx, obj, p_" + field.JsName + ", QJS.NewInt32(ctx, (int)comp." + field.Name + "));");
                        break;
                    case JsFieldType.Float2:
                        ...
                        sb.AppendLine("\t\t\t\t{");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, px, QJS.NewFloat64(ctx, comp." + field.Name + ".x));");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, py, QJS.NewFloat64(ctx, comp." + field.Name + ".y));");
                        sb.AppendLine("\t\t\t\t}");
                    ...
                    case JsFieldType.Float3:
                        ...
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, px, QJS.NewFloat64(ctx, comp." + field.Name + ".x));");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, py, QJS.NewFloat64(ctx, comp." + field.Name + ".y));");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, pz, QJS.NewFloat64(ctx, comp." + field.Name + ".z));");
                        sb.AppendLine("\t\t\t\t}");
                        sb.AppendLine("\t\t\t\tQJS.JS_SetPropertyStr(ctx, obj, p_" + field.JsName + ", sub);");
                        sb.AppendLine("\t\t\t}");
                        break;
                    case JsFieldType.Float4:
                        ...
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, py, QJS.NewFloat64(ctx, comp." + field.Name + ".y));");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, pz, QJS.NewFloat64(ctx, comp." + field.Name + ".z));");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, pw, QJS.NewFloat64(ctx, comp." + field.Name + ".w));");
                        sb.AppendLine("\t\t\t\t}");
                        sb.AppendLine("\t\t\t\tQJS.JS_SetPropertyStr(ctx, obj, p_" + field.JsName + ", sub);");
                        sb.AppendLine("\t\t\t}");
                        break;
                    case JsFieldType.Quaternion:
                        ...
                        sb.AppendLine("\t\t\t\tfixed (byte* px = s_x, py = s_y, pz = s_z, pw = s_w)");
                        sb.AppendLine("\t\t\t\t{");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, px, QJS.NewFloat64(ctx, comp." + field.Name + ".value.x));");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, py, QJS.NewFloat64(ctx, comp." + field.Name + ".value.y));");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, pz, QJS.NewFloat64(ctx, comp." + field.Name + ".value.z));");
                        sb.AppendLine("\t\t\t\t\tQJS.JS_SetPropertyStr(ctx, sub, pw, QJS.NewFloat64(ctx, comp." + field.Name + ".value.w));");
                        sb.AppendLine("\t\t\t\t}");
                        sb.AppendLine("\t\t\t\tQJS.JS_SetPropertyStr(ctx, obj, p_" + field.JsName + ", sub);");
                        sb.AppendLine("\t\t\t}");
                        break;
        ...
        static void EmitSetComponent(StringBuilder sb, ImmutableArray<FieldInfo> fields, string fullTypeName)
        {
            sb.AppendLine("\t\t[MonoPInvokeCallback(typeof(QJSShimCallback))]");
            sb.AppendLine("\t\tstatic unsafe void Set_Component(JSContext ctx, long thisU, long thisTag,");
            sb.AppendLine("\t\t\tint argc, JSValue* argv, long* outU, long* outTag)");
            sb.AppendLine("\t\t{");
            sb.AppendLine("\t\t\tint entityId;");
            sb.AppendLine("\t\t\tQJS.JS_ToInt32(ctx, &entityId, argv[0]);");
            ...
            foreach (var field in fields)
            {
                ...
                {
                    case JsFieldType.Float:
                        sb.AppendLine("\t\t\t\tvar v_" + field.JsName + " = QJS.JS_GetPropertyStr(ctx, data, p_" + field.JsName + ");");
                        sb.AppendLine("\t\t\t\tdouble d_" + field.JsName + "; QJS.JS_ToFloat64(ctx, &d_" + field.JsName + ", v_" + field.JsName + ");");
                        sb.AppendLine("\t\t\t\tcomp." + field.Name + " = (float)d_" + field.JsName + ";");
                        sb.AppendLine("\t\t\t\tQJS.JS_FreeValue(ctx, v_" + field.JsName + ");");
                        break;
                    case JsFieldType.Int:
                        sb.AppendLine("\t\t\t\tvar v_" + field.JsName + " = QJS.JS_GetPropertyStr(ctx, data, p_" + field.JsName + ");");
                        sb.AppendLine("\t\t\t\tint i_" + field.JsName + "; QJS.JS_ToInt32(ctx, &i_" + field.JsName + ", v_" + field.JsName + ");");
                        sb.AppendLine("\t\t\t\tcomp." + field.Name + " = i_" + field.JsName + ";");
                        sb.AppendLine("\t\t\t\tQJS.JS_FreeValue(ctx, v_" + field.JsName + ");");
                        break;
                    case JsFieldType.Bool:
                        sb.AppendLine("\t\t\t\tvar v_" + field.JsName + " = QJS.JS_GetPropertyStr(ctx, data, p_" + field.JsName + ");");
                        sb.AppendLine("\t\t\t\tcomp." + field.Name + " = QJS.JS_ToBool(ctx, v_" + field.JsName + ") != 0;");
                        sb.AppendLine("\t\t\t\tQJS.JS_FreeValue(ctx, v_" + field.JsName + ");");
                        break;
                    case JsFieldType.Enum:
                        sb.AppendLine("\t\t\t\tvar v_" + field.JsName + " = QJS.JS_GetPropertyStr(ctx, data, p_" + field.JsName + ");");
                        sb.AppendLine("\t\t\t\tint e_" + field.JsName + "; QJS.JS_ToInt32(ctx, &e_" + field.JsName + ", v_" + field.JsName + ");");
                        sb.AppendLine("\t\t\t\tcomp." + field.Name + " = (" + field.EnumTypeName + ")e_" + field.JsName + ";");
                        sb.AppendLine("\t\t\t\tQJS.JS_FreeValue(ctx, v_" + field.JsName + ");");
                        break;
                    case JsFieldType.Float2:
        ...
        static void EmitReadCompound(StringBuilder sb, FieldInfo field, string[] components, bool isQuaternion)
        {
            ...
            foreach (var c in components)
            {
                ...
                sb.AppendLine("\t\t\t\t{");
                sb.AppendLine("\t\t\t\t\tvar cv_" + c + " = QJS.JS_GetPropertyStr(ctx, v_" + field.JsName + ", p" + c + "_" + field.JsName + ");");
                sb.AppendLine("\t\t\t\t\tdouble cd_" + c + "; QJS.JS_ToFloat64(ctx, &cd_" + c + ", cv_" + c + ");");
                sb.AppendLine("\t\t\t\t\tcomp." + field.Name + valueSuffix + "." + c + " = (float)cd_" + c + ";");
        ...
        static void GenerateEnum(SourceProductionContext ctx, EnumTarget target)
        {
            ...
            sb.AppendLine("#endif");
            sb.AppendLine("\t\t[UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.AfterAssembliesLoaded)]");
            sb.AppendLine("\t\tstatic void AutoRegister() => JsComponentRegistry.RegisterEnum(Register);");
            sb.AppendLine();
            ...
            {
                sb.AppendLine("\t\t\tvar p" + member.Name + " = System.Text.Encoding.UTF8.GetBytes(\"" + member.Name + "\\0\");");
                sb.AppendLine("\t\t\tfixed (byte* pp" + member.Name + " = p" + member.Name + ")");
                sb.AppendLine("\t\t\t\tQJS.JS_SetPropertyStr(ctx, ns, pp" + member.Name + ", QJS.NewInt32(ctx, " + member.Value + "));");
            }
            sb.AppendLine("\t\t\tvar global = QJS.JS_GetGlobalObject(ctx);");
            sb.AppendLine("\t\t\tvar pName = System.Text.Encoding.UTF8.GetBytes(\"" + target.JsName + "\\0\");");
        ...
        struct BridgeTarget
        {
            ...
            public BridgeTarget(string typeName, string fullTypeName, string jsName, bool needAccessors, bool needSetters, ImmutableArray<FieldInfo> fields, string description = null)
            {
                TypeName = typeName;
        ...
        struct FieldInfo
        {
            ...
            public FieldInfo(string name, string jsName, JsFieldType type, bool needAccessors, bool needSetters, string enumTypeName = null, string description = null)
            {
                Name = name;
                JsName = jsName;
                Type = type;
        ...
        struct EnumTarget
        {
            ...
            public EnumTarget(string typeName, string jsName, ImmutableArray<EnumMember> members, string description)
            {
                TypeName = typeName;
                JsName = jsName;
                Members = members;

namespace UnityJS.Entities.PlayModeTests
{
	...
	public class JsScriptSourceRegistryE2ETests
	{
		...
		public IEnumerator TearDown()
		{
			...
			{
				try { Directory.Delete(m_TempDir, true); } catch { /* best effort */ }
				m_TempDir = null;
			}
		...
		{
			m_TempDir = Path.Combine(Path.GetTempPath(), "unity_js_test_" + Guid.NewGuid().ToString("N")[..8]);
		...
		public IEnumerator Register_FileSystemSource_DiscoversSystems()
		{
			...
			{
				if (name == "test_reg_a") { found = true; break; }
			}
		...
		public IEnumerator Register_BundleSource_DiscoversSystems()
		{
			...
			bundle.Add("systems/bundle_sys", "export function onUpdate(state) { globalThis._e2eBundleSys = true; }");
			...
			foreach (var (name, _) in systems)
			{
				if (name == "bundle_sys") { found = true; break; }
			}
		...
		public IEnumerator Register_MultipleSourcesPriorityOrder()
		{
			...
			bundle1.Add("systems/prio_test", "export function onUpdate(state) { globalThis._e2ePrio = 'high'; }");

			var bundle2 = new BundleScriptSource("prio-low", 100);
			bundle2.Add("systems/prio_test", "export function onUpdate(state) { globalThis._e2ePrio = 'low'; }");
			...
			{
				if (name == "prio_test") { winner = src; break; }
			}
		...
		public IEnumerator Unregister_SourceRemoved_NotDiscoveredAnymore()
		{
			...
			foreach (var (name, _) in systems)
			{
				if (name == "unreg_sys") { found = true; break; }
			}
		...
		public IEnumerator Reset_ClearsAllSources()
		{
			...
			foreach (var (name, _) in systems)
			{
				if (name == "reset_sys") { found = true; break; }
			}
		...
		{
			var dir = CreateTempDir();
			File.WriteAllText(Path.Combine(dir, "my_script.js"),
				"export function onInit(state) { globalThis._e2eFsRead = true; }");

			var source = new FileSystemScriptSource("test-read", dir, 10);
			JsScriptSourceRegistry.Register(source);

			Assert.IsTrue(JsScriptSourceRegistry.TryReadScript("my_script", out var code, out var resolvedId));
		...
		public IEnumerator TryReadScript_BundleSource_ReadsFromMemory()
		{
			...
			bundle.Add("my_mem_script", "export function onInit(state) { globalThis._e2eMemRead = true; }");
			JsScriptSourceRegistry.Register(bundle);

			Assert.IsTrue(JsScriptSourceRegistry.TryReadScript("my_mem_script", out var code, out var resolvedId));
		...
		public IEnumerator Fulfillment_ScriptFromBundleSource()
		{
			...
			bundle.Add("test_fulfill_bundle",
			...
			m_EntityManager.AddComponentData(entity, new LocalTransform
			{
				Position = float3.zero,
				Rotation = quaternion.identity,
				Scale = 1f
			});
			m_EntityManager.AddComponentData(entity, new JsEntityId { value = JsEntityRegistry.IsCreated ? JsEntityRegistry.AllocateId() : 1 });
			...
			Assert.IsTrue(GetGlobalBool("_e2eFulfillBundle"), "onInit should have been called via fulfillment");
		...
		public IEnumerator Import_RelativePath_BundleSource()
		{
			...
			bundle.Add("systems/bundle_main",
				"import { helper } from './bundle_helper.js';\n" +
				"export function onUpdate(state) { globalThis._e2eBundleImport = helper(); }");
			bundle.Add("systems/bundle_helper",
				"export function helper() { return 77; }");
			JsScriptSourceRegistry.Register(bundle);

			Assert.IsTrue(bundle.TryReadScript("systems/bundle_main", out var source, out var resolvedId));
			...
			Assert.AreEqual(77, GetGlobalInt("_e2eBundleImport"),
		...
		public IEnumerator Import_BareModule_FileSystem()
		{
			...
			Assert.AreEqual(99, GetGlobalInt("_e2eBareImport"), "Module import should resolve and execute");
		...
		public IEnumerator DuplicateSourceId_ReplacesExisting()
		{
			...
			bundle1.Add("systems/dup_sys", "export function onUpdate(state) { globalThis._e2eDup = 'first'; }");
			JsScriptSourceRegistry.Register(bundle1);

			var bundle2 = new BundleScriptSource("dup-id", 10);
			bundle2.Add("systems/dup_sys_v2", "export function onUpdate(state) { globalThis._e2eDup = 'second'; }");
			JsScriptSourceRegistry.Register(bundle2);
			...
			foreach (var (name, _) in systems)
			{
				if (name == "dup_sys") foundV1 = true;
				if (name == "dup_sys_v2") foundV2 = true;
			}
		...
		public IEnumerator EmptySource_DiscoverReturnsEmpty()
		{
			...
			foreach (var (name, src) in systems)
			{
				if (src.SourceId == "test-empty") { found = true; break; }
			}
		...
		public IEnumerator FileSystemSource_SubdirDiscovery()
		{
			...
			foreach (var (name, _) in systems)
			{
				if (name == "sub_sys_a") foundA = true;
				if (name == "sub_sys_b") foundB = true;
			}
		...
		public IEnumerator TryReadModuleBytes_BundlePath_Works()
		{
			...
			Assert.IsTrue(JsScriptSourceRegistry.TryReadModuleBytes(
				"bundle://test-bytes/utils/data.js", out var data));
		...
		public IEnumerator TryFindModule_FileSystemSource_ReturnsAbsolutePath()
		{
			...
			Assert.IsTrue(Path.IsPathRooted(resolved), "Filesystem module should resolve to absolute path");
		...
		public IEnumerator TryFindModule_BundleSource_ReturnsBundlePath()
		{
			...
			Assert.IsTrue(resolved.StartsWith("bundle://"), "Bundle module should resolve to bundle:// path");

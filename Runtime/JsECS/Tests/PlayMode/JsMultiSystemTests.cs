namespace UnityJS.Entities.PlayModeTests
{
	...
	public class JsMultiSystemTests
	{
		...
		(string scriptId, int stateRef) LoadInlineScript(string scriptId, string source, int entityId = -1)
		{
			var filename = $"<test>/{scriptId}_{m_UniqueCounter++}.js";
			Assert.IsTrue(m_Vm.LoadScriptAsModule(scriptId, source, filename),
		...
		unsafe void EvalGlobal(string code)
		{
			...
			fixed (byte* pSource = sourceBytes, pFilename = filenameBytes)
			{
				...
				if (QJS.IsException(val))
					Debug.LogError("[Test] EvalGlobal exception");
		...
		static int ComputeFixedTicks(float[] frameDts, float fixedDt)
		{
			...
			{
				acc += dt;
				while (acc >= fixedDt) { count++; acc -= fixedDt; }
			}
			return count;
		}
		...
		public IEnumerator MultipleScripts_StateIsolation()
		{
			for (var i = 0; i < 4; i++)
			{
				var src = $"export function onTick(state) {{ globalThis._iso_{i} = (globalThis._iso_{i} || 0) + 1; }}";
		...
		public IEnumerator MultipleScripts_SharedGlobal()
		{
			for (var i = 0; i < 3; i++)
			{
				var src = "export function onTick(state) { globalThis._shared = (globalThis._shared || 0) + 1; }";
		...
		public IEnumerator TickGroupFiltering_AllFiveGroups_Parsed()
		{
			...
			foreach (var (annotation, expected) in groups)
			{
				...
				var ann = JsScriptAnnotationParser.Parse(source);
				Assert.AreEqual(expected, ann.tickGroup, $"Annotation '{annotation}' should parse to {expected}");
		...
		public IEnumerator ExecutionOrder_WithinSameGroup_Deterministic()
		{
			...
			{
				var label = labels[i];
				var src = $"export function onTick(state) {{ globalThis._log = (globalThis._log || '') + '{label}'; }}";
		...
		{
			var entitySrc = "export function onTick(state) { globalThis._log = (globalThis._log || '') + 'E'; }";
			var systemSrc = "export function onUpdate(state) { globalThis._log = (globalThis._log || '') + 'S'; }";
		...
		public IEnumerator ExecutionOrder_MixedTickGroups_CorrectPhaseOrder()
		{
			...
			var sysSrc = "export function onUpdate(state) { globalThis._log = (globalThis._log || '') + 'SR,'; }";
		...
		public IEnumerator FixedVsVariable_UniformFrames()
		{
			...
			for (var i = 0; i < 60; i++) frameDts[i] = 0.033f;
			const float fixedDt = 0.02f;

			var entries = new List<ScriptEntry>
			{
				new() { scriptId = varS.scriptId, stateRef = varS.stateRef, group = JsTickGroup.Variable },
				new() { scriptId = fixS.scriptId, stateRef = fixS.stateRef, group = JsTickGroup.Fixed },
			};
		...
		public IEnumerator FixedVsVariable_VariableFramerate()
		{
			...
			for (var i = 0; i < 20; i++) frameDts[i] = 0.008f;
			for (var i = 20; i < 40; i++) frameDts[i] = 0.05f;
			const float fixedDt = 0.02f;
		...
		public IEnumerator FixedVsVariable_LargeSpike()
		{
			...
			for (var i = 0; i < 9; i++) frameDts[i] = 0.016f;
			...
			Assert.GreaterOrEqual(spikeTicks, 10, $"Spike frame should produce >=10 fixed steps, got {spikeTicks}");
			yield return null;
		}
		...
		public IEnumerator FixedVsVariable_DtAccumulation()
		{
			...
			var frameDts = new[]
			{
				0.016f, 0.033f, 0.025f, 0.050f, 0.008f,
				0.016f, 0.033f, 0.025f, 0.050f, 0.008f,
				0.040f, 0.012f, 0.028f, 0.045f, 0.010f,
				0.016f, 0.033f, 0.025f, 0.050f, 0.008f,
				0.040f, 0.012f, 0.028f, 0.045f, 0.010f,
				0.016f, 0.033f, 0.025f, 0.050f, 0.008f,
			};
			...
			foreach (var dt in frameDts) expectedVarDt += dt;
		...
		{
			var src = "export function onTick(state) { globalThis._dtLog = (globalThis._dtLog || '') + state.deltaTime.toFixed(4) + ','; }";
		...
		public IEnumerator VariableDt_ManyScripts_AllReceiveSameDt()
		{
			...
			foreach (var frameDt in frameDts)
			{
				...
				for (var i = 0; i < scriptCount; i++)
					Assert.AreEqual(frameDt, GetGlobalFloat($"_dtLoad_{i}"), 0.001f,
		...
		public IEnumerator SystemAndEntity_FixedEntityVariableSystem()
		{
			...
			var sysSrc = "export function onUpdate(state) { globalThis._sysCount = (globalThis._sysCount || 0) + 1; }";

			var fixS = LoadInlineScript("sefx_entity", fixSrc);
			var sysS = LoadInlineScript("sefx_system", sysSrc);

			var frameDts = new float[10];
			for (var i = 0; i < 10; i++) frameDts[i] = 0.033f;
			const float fixedDt = 0.02f;
		...
		public IEnumerator Stress_50Scripts_100Frames()
		{
			...
			for (var i = 0; i < scriptCount; i++)
			{
				var src = $"export function onTick(state) {{ globalThis._stress_{i} = (globalThis._stress_{i} || 0) + 1; }}";
		...
		{
			var enabledSrc = "export function onTick(state) { globalThis._enabled = (globalThis._enabled || 0) + 1; }";
			var disabledSrc = "export function onTick(state) { globalThis._disabled = (globalThis._disabled || 0) + 1; }";

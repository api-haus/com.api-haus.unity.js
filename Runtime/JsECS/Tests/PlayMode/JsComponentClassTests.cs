namespace UnityJS.Entities.PlayModeTests
{
  ...
  public unsafe class JsComponentClassTests : JsBridgeTestFixture
  {
    ...
    public void ModuleLoad_ComponentClass_InitWorks()
    {
      ...
      Assert.IsTrue(m_Manager.LoadScriptAsModule(scriptId, source, scriptPath),
...
";
      Assert.IsTrue(m_Manager.LoadScriptAsModule("test_tick_module", source, "<test_tick>"),
        "Module should load without errors");
      ...
      Assert.AreEqual(3, ticks, "TickComponents should have ticked the module-loaded instance 3 times");
    }

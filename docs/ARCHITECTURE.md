...
| **TSC compilation** | TypeScript → JavaScript | `TscCompiler.cs` | Keep as-is (clean) |
...
| File | Responsibility |
|------|---------------|
| `Editor/TscCompiler.cs` | TypeScript compilation |
| `Editor/TscFileWatcher.cs` | Watch .ts changes |
...
A `tsconfig.json` is auto-generated at project root including all `.ts` files.

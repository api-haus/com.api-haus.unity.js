...
namespace UnityJS.Editor
{
	static class TscCompiler
	{
		static string TsconfigPath =>
			Path.GetFullPath(Path.Combine(Application.dataPath, "StreamingAssets", "unity.js", "tsconfig.json"));
		...
		static bool ShouldCompile()
		{
			...
			var tsDir = Path.GetFullPath(Path.Combine(Application.dataPath, "StreamingAssets", "unity.js", "systems"));
			...
			{
				var relative = ts.Substring(tsDir.Length).TrimStart(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
